...
- (NSString *)stringFromAdType:(TDAdTypes)adType {
    switch (adType) {
...
- (void)onRedeemFailed:(TDAdRequest * _Nonnull)adRequest didFailToValidateReward:(TDReward * _Nonnull)reward {
    [self sendEvent:[NSString stringWithFormat:@"Failed to validate reward for ad unit - %@ for tag - %@\nReward:\n    ID: %@\n    Name: %@\n    Amount: %i\n    Is valid: %@\n    Hashed user ID: %@\n    Custom JSON:\n%@", [self stringFromAdType:adRequest.placement.adTypes], adRequest.placement.tag, reward.rewardId, reward.name, reward.value, reward.isValid ? @"true" : @"false", reward.hashedUserId, reward.customJson]];
}
